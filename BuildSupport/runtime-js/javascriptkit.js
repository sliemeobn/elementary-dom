...
class SwiftRuntime {
  ...
  get closureDeallocator() {
    ...
    const librarySupportsWeakRef =
      (features & 1) /* LibraryFeatures.WeakRefs */ != 0;
  ...
  get wasmImports() {
    ...
    const getMessageBroker = (threadChannel) => {
      ...
      const newBroker = new MessageBroker(
        (_a = this.tid) !== null && _a !== void 0 ? _a : -1,
        threadChannel,
        {
          onRequest: (message) => {
            let returnValue;
            try {
              // @ts-ignore
              const result = itcInterface[message.data.request.method](
            ...
            try {
              ...
              responseMessage.data.response = {
                ...
                error: serializeError(
                  new TypeError(`Failed to serialize message: ${error}`)
                ),
              };
              newBroker.reply(responseMessage);
            }
    ...
    return {
      ...
      swjs_release_remote: (tid, ref) => {
        ...
        broker.request({
          ...
          data: {
            sourceTid:
              (_a = this.tid) !== null && _a !== void 0 ? _a : MAIN_THREAD_TID,
            targetTid: tid,
            context: 0,
            request: {
              method: "release",
      ...
      swjs_listen_message_from_main_thread: () => {
        ...
        if (
          !(threadChannel && "listenMessageFromMainThread" in threadChannel)
        ) {
          throw new Error(
      ...
      swjs_listen_message_from_worker_thread: (tid) => {
        const threadChannel = this.options.threadChannel;
        if (
          !(threadChannel && "listenMessageFromWorkerThread" in threadChannel)
        ) {
          throw new Error(
      ...
      swjs_terminate_worker_thread: (tid) => {
        ...
        if (threadChannel && "terminateWorkerThread" in threadChannel) {
          (_a = threadChannel.terminateWorkerThread) === null || _a === void 0
            ? void 0
            : _a.call(threadChannel, tid);
